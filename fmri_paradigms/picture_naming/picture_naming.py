...
expName = 'picture_naming'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\swastlin\\Desktop\\fmri_paradigms\\picture_naming\\picture_naming.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions_wait_for_manual_trigger = visual.ImageStim(
        ...
        image='resources/instructions_picture_naming.png', mask=None, anchor='center',
    ...
    # Initialise counter to loop through symbols lists
    symbols_counter = 0
    rest_rep_counter = 0
    ...
    objects_dp = resources_dp / 'objects'
    objects = [x for x in objects_dp.iterdir() if x.is_file()]
    
    # Shuffle list of objects
    random.shuffle(objects)
    
    # Initialise counters to loop through object list
    object_counter = 0
    task_rep_counter = 0
    ...
    for thisBlock in blocks:
        ...
        for thisRepeats_rest in repeats_rest:
            ...
            while continueRoutine:
                ...
                if rest_crosshair.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > rest_crosshair.tStartRefresh + 1.0-frameTolerance:
                        # keep track of stop time/frame for later
        ...
        for thisRepeats_task in repeats_task:
            ...
            task_block = data.Routine(
                name='task_block',
                components=[task_crosshair, object],
            )
            task_block.status = NOT_STARTED
            continueRoutine = True
            # update component parameters for each repeat
            # Run 'Begin Routine' code from task_code
            # Set duration object appears on screen
            if task_rep_counter < 8: 
                object_duration = 1
            else:
                object_duration = 1.2
            object.setImage(objects[object_counter])
            ...
            while continueRoutine:
                ...
                if task_crosshair.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if task_crosshair.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > task_crosshair.tStartRefresh + 1.0-frameTolerance:
                        # keep track of stop time/frame for later
                ...
                # if object is starting this frame...
                if object.status == NOT_STARTED and tThisFlip >= 1-frameTolerance:
                    # keep track of start time/frame for later
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'object.started')
                    # update status
                    object.status = STARTED
                    object.setAutoDraw(True)
                
                # if object is active this frame...
                if object.status == STARTED:
                    # update params
                    pass
                
                # if object is stopping this frame...
                if object.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > object.tStartRefresh + object_duration-frameTolerance:
                        # keep track of stop time/frame for later
                        object.tStop = t  # not accounting for scr refresh
                        object.tStopRefresh = tThisFlipGlobal  # on global time
                        object.frameNStop = frameN  # exact frame index
                        # add timestamp to datafile
                        thisExp.timestampOnFlip(win, 'object.stopped')
                        # update status
            ...
            # Increment object_counter and task_rep_counter
            object_counter += 1
            if task_rep_counter < 8: 
                task_rep_counter += 1
            else:
