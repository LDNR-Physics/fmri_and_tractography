...
# Run 'Before Experiment' code from rest_task_code
import random
...
expName = 'verbal_fluency'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\swastlin\\Desktop\\fmri_paradigms\\verbal_fluency\\verbal_fluency.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    instructions_wait_for_manual_trigger = visual.ImageStim(
        ...
        image='resources/verbal_fluency_instructions.png', mask=None, anchor='center',
    ...
    # --- Initialize components for Routine "rest_task_block" ---
    # Run 'Begin Experiment' code from rest_task_code
    # Shuffle list of letters
    letters = list("ABCDEFGLMNOPRSTUVZ")
    ...
    for thisBlock in blocks:
        ...
        # --- Prepare to start Routine "rest_task_block" ---
        # create an object to store info about Routine rest_task_block
        rest_task_block = data.Routine(
            name='rest_task_block',
            components=[rest, task],
        )
        rest_task_block.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        task.setText(letters[blocks.thisN])
        # store start times for rest_task_block
        rest_task_block.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        rest_task_block.tStart = globalClock.getTime(format='float')
        rest_task_block.status = STARTED
        thisExp.addData('rest_task_block.started', rest_task_block.tStart)
        rest_task_block.maxDuration = None
        # keep track of which components have finished
        rest_task_blockComponents = rest_task_block.components
        for thisComponent in rest_task_block.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "rest_task_block" ---
        # if trial has changed, end Routine now
        if isinstance(blocks, data.TrialHandler2) and thisBlock.thisN != blocks.thisTrial.thisN:
            continueRoutine = False
        rest_task_block.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine and routineTimer.getTime() < 36.4:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if task.status == NOT_STARTED and tThisFlip >= 18.2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if task.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > task.tStartRefresh + 18.2-frameTolerance:
                    # keep track of stop time/frame for later
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                rest_task_block.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in rest_task_block.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "rest_task_block" ---
        for thisComponent in rest_task_block.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for rest_task_block
        rest_task_block.tStop = globalClock.getTime(format='float')
        rest_task_block.tStopRefresh = tThisFlipGlobal
        thisExp.addData('rest_task_block.stopped', rest_task_block.tStop)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if rest_task_block.maxDurationReached:
            routineTimer.addTime(-rest_task_block.maxDuration)
        elif rest_task_block.forceEnded:
            routineTimer.reset()
        else:
            routineTimer.addTime(-36.400000)
