...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\swastlin\\Desktop\\fmri_paradigms\\visual\\visual.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # This variable needs to be available with "run" function to must be placed here rather than in "Before Experiment"
    ...
    instructions_wait_for_manual_trigger = visual.ImageStim(
        ...
        image='resources/visual_instructions.png', mask=None, anchor='center',
    ...
    rest_crosshair = visual.ImageStim(
        win=win,
        name='rest_crosshair', units='height', 
        image='resources/visual_rest.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1.3, None),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=0.0)
    ...
    # Generate a list of periods to display each chequerboard image
    frame = 0
    
    # Set length of block
    block_duration_ms = 15600
    
    # Calculate number of frames to make this block length
    max_frames = int(block_duration_ms * expInfo['frameRate'] / 1000)
    
    # Calculate difference between desired length of block and actual
    actual_block_duration_s = max_frames / expInfo['frameRate']
    actual_block_duration_ms = 1000 * actual_block_duration_s
    pad_duration = (block_duration_ms - actual_block_duration_ms) / 1000
    
    # Set minimum time to display an image 
    min_flick_duration = 0.05 
    min_flick_frames = int(min_flick_duration * expInfo['frameRate'])
    
    # Set maximum time to display and image
    max_flick_duration = 0.25
    max_flick_frames = int(max_flick_duration * expInfo['frameRate'])
    
    frame = 0
    frame_list_a = []
    while True:
        frame1 = random.randint(min_flick_frames,max_flick_frames)
        frame += frame1
        frame_list_a.append(frame1)
        if frame >= (max_frames - max_flick_frames):
            break
        
    # Display one more image to stop at max_frames exactly
    frames_remaining = max_frames - frame
    frame_list_a.append(frames_remaining)
    num_frames = len(frame_list_a)
    
    # Create a second shuffled list for the other image
    frame_list_b = frame_list_a.copy()
    random.shuffle(frame_list_b)
    
    counter = 0
    task_a = visual.ImageStim(
        win=win,
        name='task_a', units='height', 
        image='resources/visual_chequerboard_a.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1.3, None),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    task_b = visual.ImageStim(
        ...
        image='resources/visual_chequerboard_b.png', mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1.3, None),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-2.0)
    
    # --- Initialize components for Routine "task_pad" ---
    ...
    for thisBlock in blocks:
        ...
        while continueRoutine and routineTimer.getTime() < 15.6:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            tThisFlipGlobal = win.getFutureFlipTime(clock=None)
            ...
            if rest_crosshair.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                # keep track of start time/frame for later
            ...
            if rest_crosshair.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > rest_crosshair.tStartRefresh + 15.6-frameTolerance:
                    # keep track of stop time/frame for later
        ...
        if rest_block.maxDurationReached:
            routineTimer.addTime(-rest_block.maxDuration)
        elif rest_block.forceEnded:
            routineTimer.reset()
        else:
            routineTimer.addTime(-15.600000)
        ...
        for thisRepeats_task in repeats_task:
            ...
            while continueRoutine:
                ...
                if task_a.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
                    # keep track of start time/frame for later
                ...
                if task_a.status == STARTED:
                    if frameN >= (task_a.frameNStart + frame_list_a[counter]):
                        # keep track of stop time/frame for later
                ...
                if task_b.status == NOT_STARTED and frameN >= frame_list_a[counter]:
                    # keep track of start time/frame for later
                ...
                if task_b.status == STARTED:
                    if frameN >= (task_b.frameNStart + frame_list_b[counter]):
                        # keep track of stop time/frame for later
            ...
            if counter == num_frames:
                random.shuffle(frame_list_a)
                random.shuffle(frame_list_b)
                counter = 0
            # the Routine "task_block" was not non-slip safe, so reset the non-slip timer
            routineTimer.reset()
        # completed num_frames repeats of 'repeats_task'
        
        
        # --- Prepare to start Routine "task_pad" ---
        # create an object to store info about Routine task_pad
        task_pad = data.Routine(
            name='task_pad',
            components=[],
        )
        task_pad.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from task_pad_code
        core.wait(pad_duration)
        # store start times for task_pad
        task_pad.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        task_pad.tStart = globalClock.getTime(format='float')
        task_pad.status = STARTED
        thisExp.addData('task_pad.started', task_pad.tStart)
        task_pad.maxDuration = None
        # keep track of which components have finished
        task_padComponents = task_pad.components
        for thisComponent in task_pad.components:
            thisComponent.tStart = None
            thisComponent.tStop = None
            thisComponent.tStartRefresh = None
            thisComponent.tStopRefresh = None
        ...
        # --- Run Routine "task_pad" ---
        # if trial has changed, end Routine now
        if isinstance(blocks, data.TrialHandler2) and thisBlock.thisN != blocks.thisTrial.thisN:
            continueRoutine = False
        task_pad.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            # get current time
            t = routineTimer.getTime()
            tThisFlip = win.getFutureFlipTime(clock=routineTimer)
            ...
            # check if all components have finished
            if not continueRoutine:  # a component has requested a forced-end of Routine
                task_pad.forceEnded = routineForceEnded = True
                break
            continueRoutine = False  # will revert to True if at least one component still running
            for thisComponent in task_pad.components:
                if hasattr(thisComponent, "status") and thisComponent.status != FINISHED:
        ...
        # --- Ending Routine "task_pad" ---
        for thisComponent in task_pad.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for task_pad
        task_pad.tStop = globalClock.getTime(format='float')
        task_pad.tStopRefresh = tThisFlipGlobal
        thisExp.addData('task_pad.stopped', task_pad.tStop)
        # the Routine "task_pad" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
